...
export const drinkChoice = pgEnum("drink_choice", [
	"Tea",
	"Coffee",
	"Green tea",
	"Milk",
...
export const drinkSource = pgEnum("drink_source", [
	"default",
	"manual",
	"admin",
]);
export const userRole = pgEnum("user_role", ["user", "admin", "guest"]);
export const guestStatus = pgEnum("guest_status", [
	"pending",
	"approved",
	"rejected",
]);
...
export const companyAdmin = pgTable(
	...
	(table) => [
		...
		index("company_admin_email_idx").on(table.email),
	],
);
