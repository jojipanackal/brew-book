	...
	const guest = await db
		...
		.where(
			and(
				...
				gt(user.guestExpiresAt, new Date()),
			),
		)
		.limit(1);
	return guest[0] ?? null;
...
function defaultsFromRows(
	rows: Array<{ period: Period; drink: Drink; sugar: boolean }>,
): { defaults: DrinkChoice; sugarDefaults: SugarChoice } {
	...
	for (const row of rows) {
		defaults[row.period] = row.drink;
		sugarDefaults[row.period] = row.sugar;
	}
...
export async function readDay(userId: string | undefined, date: string) {
	const [defaultRows, currentUserRows] = await Promise.all([
		userId
			? db
					.select({
						period: drinkDefault.period,
						drink: drinkDefault.drink,
						sugar: drinkDefault.sugar,
					})
					.from(drinkDefault)
					.where(eq(drinkDefault.userId, userId))
			: Promise.resolve([]),
		userId
			? db
					.select({ companyId: user.companyId })
	...
	const responseRows = await db
		...
		.where(
			companyId
				? and(eq(drinkResponse.date, date), eq(user.companyId, companyId))
				: eq(drinkResponse.date, date),
		);
	...
	const grouped = new Map<
		string,
		{
			...
			choices: Partial<DrinkChoice>;
			sugar: Partial<SugarChoice>;
			sources: Partial<Record<Period, PollSource>>;
		}
	>();
	...
	return {
		...defaultSettings,
		responses: [...grouped.values()].map((entry) => ({
			user: entry.user,
			choices: {
				morning: entry.choices.morning ?? "No drink",
				evening: entry.choices.evening ?? "No drink",
			},
			sugar: {
				morning: entry.sugar.morning ?? true,
				evening: entry.sugar.evening ?? true,
			},
			sources: {
				morning: entry.sources.morning ?? "default",
				evening: entry.sources.evening ?? "default",
			},
		})),
	};
}
...
export async function ensureTodayResponses(companyId: string, date: string) {
	...
	const eligibleMembers = members.filter(
		(member) => !member.isGuest || member.guestStatus === "approved",
	);
	...
	const defaultRows = await db
		.select({
			userId: drinkDefault.userId,
			period: drinkDefault.period,
			drink: drinkDefault.drink,
			sugar: drinkDefault.sugar,
		})
		.from(drinkDefault)
		.where(inArray(drinkDefault.userId, memberIds));
	const defaultsByUser = new Map<
		string,
		Partial<Record<Period, { drink: Drink; sugar: boolean }>>
	>();
	...
	server: {
		handlers: {
			GET: async ({ request }) => {
				...
				const day =
					requestedDate === getDateKey()
						? await readDay(currentUser.id, requestedDate)
						: beforeEnsure;
			...
			PUT: async ({ request }) => {
				...
				if (body.type === "default") {
					if (currentUser.email.endsWith("@guest.brewbook.local"))
						return json(
							{ error: "Guests cannot set defaults" },
							{ status: 403 },
						);
					if (
						!isPeriod(body.period) ||
						!isDrink(body.drink) ||
						typeof body.sugar !== "boolean"
					)
						return json({ error: "Invalid default" }, { status: 400 });
					...
					await db
						...
						.onConflictDoUpdate({
							target: [drinkDefault.userId, drinkDefault.period],
							set: { drink: body.drink, sugar, updatedAt: new Date() },
						});
					await db
						.update(drinkResponse)
						.set({ drink: body.drink, sugar, updatedAt: new Date() })
						.where(
							and(
								eq(drinkResponse.userId, currentUser.id),
								eq(drinkResponse.date, getDateKey()),
								eq(drinkResponse.period, body.period),
								eq(drinkResponse.source, "default"),
							),
						);
					const rows = await db
						.select({
						...
						.from(drinkDefault)
						.where(eq(drinkDefault.userId, currentUser.id));
				...
				if (body.type === "response") {
					...
					if (body.date !== getDateKey())
						return json(
							{ error: "Responses can only be changed for today" },
							{ status: 400 },
						);
					...
					await db
						...
						.onConflictDoUpdate({
							target: [
								drinkResponse.userId,
								drinkResponse.date,
								drinkResponse.period,
							],
							set: {
								drink: body.drink,
								sugar,
