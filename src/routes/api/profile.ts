...
function defaultsFromRows(
	rows: Array<{ period: "morning" | "evening"; drink: Drink; sugar: boolean }>,
 ...
 */
async function claimGuestResponses(
	request: Request,
	...
	const matchedCompany = companyRows.find((item) =>
		[item.emailEnding1, item.emailEnding2].some(
			(ending) => ending && email.endsWith(ending.trim().toLowerCase()),
		),
	);
...
async function readProfile(currentUser: { id: string; email: string }) {
	const [userRow, defaultRows, companyRows, adminRows] = await Promise.all([
		...
		db
			...
			.from(company),
		db
			.select({ email: companyAdmin.email })
			.from(companyAdmin)
			.where(sql`lower(${companyAdmin.email}) = lower(${currentUser.email})`),
	]);
	const email = currentUser.email.trim().toLowerCase();
	const matchedCompany = companyRows.find((item) =>
		[item.emailEnding1, item.emailEnding2].some(
	...
	if (
		matchedCompany &&
		(userRow[0]?.companyId !== matchedCompany.id ||
			userRow[0]?.legacyCompany !== matchedCompany.name)
	) {
		await db
			.update(user)
			.set({
	...
	return {
		...
		accessDenied,
		needsOnboarding: !accessDenied && defaultRows.length < periods.length,
		role: isAdmin ? ("admin" as const) : ("user" as const),
		...defaultsFromRows(defaultRows),
	};
}
		...
		handlers: {
			...
			PUT: async ({ request }) => {
				...
				const body = (await request.json()) as {
					company?: unknown;
					defaults?: Partial<Record<"morning" | "evening", unknown>>;
					sugarDefaults?: Partial<Record<"morning" | "evening", unknown>>;
				};
				const profile = await readProfile(currentUser);
				const companyName = profile.company;
				if (!isCompany(companyName) || profile.accessDenied)
					return json(
						{
							error: "Your work email is not registered with a MyBev company.",
						},
						{ status: 403 },
					);
				...
				if (
					!defaults ||
					!isDrink(defaults.morning) ||
					!isDrink(defaults.evening)
				)
					return json({ error: "Choose both default drinks" }, { status: 400 });
				const validatedDefaults: DrinkChoice = {
				...
				if (
					!sugarDefaults ||
					typeof sugarDefaults.morning !== "boolean" ||
					typeof sugarDefaults.evening !== "boolean"
				)
					return json({ error: "Choose sugar preferences" }, { status: 400 });
				const validatedSugarDefaults: SugarChoice = {
					morning:
						validatedDefaults.morning === "No drink"
							? true
							: sugarDefaults.morning,
					evening:
						validatedDefaults.evening === "No drink"
							? true
							: sugarDefaults.evening,
				};

				await db.transaction(async (tx) => {
					await tx
						.update(user)
						.set({ company: companyName, updatedAt: new Date() })
						.where(eq(user.id, currentUser.id));
					for (const period of periods) {
						await tx
							...
							.values({
								...
								period,
								drink: validatedDefaults[period],
								sugar: validatedSugarDefaults[period],
							})
							.onConflictDoUpdate({
